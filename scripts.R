...
calc_height <- 0 + (nrow(AUC_mat) * 0.25) 
calc_width <- 7 

pdf(file.path(fig.path, "AUC.pdf"), width = calc_width, height = calc_height)
draw(hm, 
...
tiff(file.path(fig.path, "AUC.tiff"), 
     width = calc_width, height = calc_height, units = "in", res = 1200, compression = "lzw")
draw(hm, 
     heatmap_legend_side = "bottom", 
     annotation_legend_side = "bottom",
     padding = unit(c(2, 2, 2, 2), "cm"))
